...
import {Armchair,Film,Users,UserPlus,Link,Globe,ArrowRight,Maximize,Play,Pause,Volume2,VolumeX,Upload,Repeat,X,ArrowLeft,MessageSquare,Send,Search,RefreshCcw,Monitor,LayoutGrid,Gauge,UserRound,Focus,Bell,Footprints} from 'lucide-react';
...
export default function Home(){
 ...
 return <main className="darling-app cinema-app">
  ...
  {panel&&<div className="dc-backdrop" onClick={close} data-ui-control><div className="dc-panel" ref={dialog} role="dialog" aria-modal="true" aria-labelledby="panel-title" onClick={e=>e.stopPropagation()} onKeyDown={e=>{if(e.key!=='Tab')return;const all=dialog.current?.querySelectorAll<HTMLElement>('button:not(:disabled),input,select,[tabindex="0"]');if(!all?.length)return;if(e.shiftKey&&document.activeElement===all[0]){e.preventDefault();all[all.length-1].focus();}else if(!e.shiftKey&&document.activeElement===all[all.length-1]){e.preventDefault();all[0].focus();}}}>
   <button className="dc-close" aria-label="Close panel" onClick={close}><X/></button>
   {panel!=='party'&&<><span className="dc-eyebrow">{panel==='profile'?'YOUR PLAYER':panel==='settings'?'THE FINER DETAILS':panel==='seats'?'MAKE YOURSELF COMFORTABLE':'DARLING CINEMAS'}</span><h2 id="panel-title">{panelNames[panel]}</h2></>}
   {panel==='player'&&<><p>Bring your own video. Your film stays on your device.</p>{media.filename&&<div className="dc-media"><Film size={30}/><strong>{title}</strong><input aria-label="Player seek" type="range" min="0" max={media.duration||1} value={Math.min(media.progress,media.duration||1)} step=".1" onChange={e=>media.seek(+e.target.value)}/><div className="dc-media-row"><span>{time(media.progress)} / {time(media.duration)}</span><button aria-label={media.isPlaying?'Pause video':'Play video'} onClick={media.togglePlay}>{media.isPlaying?<Pause/>:<Play/>}</button><button aria-label="Player mute" onClick={media.toggleMute}>{media.isMuted?<VolumeX/>:<Volume2/>}</button><input aria-label="Volume" type="range" min="0" max="1" step=".05" value={media.volume} onChange={e=>media.changeVolume(+e.target.value)}/></div><div className="dc-media-row"><label>Speed <select aria-label="Playback speed" value={media.playbackRate} onChange={e=>media.changePlaybackRate(+e.target.value)}>{[.5,.75,1,1.25,1.5,2].map(v=><option key={v} value={v}>{v===1?'Normal':`${v}×`}</option>)}</select></label><button aria-pressed={media.isLooping} onClick={media.toggleLoop}><Repeat size={14}/> Loop {media.isLooping?'on':'off'}</button><button onClick={()=>setPanel('seats')}><Armchair size={14}/> Choose a seat</button></div></div>}<div className="dc-tabs"><span className="selected">My video</span><button disabled title="Remote video sources are unavailable">Video link · Coming soon</button></div><div className="dc-drop" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();if(e.dataTransfer.files[0])load(e.dataTransfer.files[0]);}}><Upload/><h3>Bring your own story.</h3><p>Drop a video here, or choose one from your device.</p><button className="dc-pill" disabled={media.status==='loading'} onClick={()=>input.current?.click()}>{media.status==='loading'?'Preparing video…':media.filename?'Change video':'Choose a video'} →</button><small>MP4 and WebM. MOV where supported.<br/>Nothing is uploaded.</small></div>{media.status==='error'&&<p role="alert" className="dc-error">{media.error||'This video could not be played. Try an MP4 with H.264 video or a WebM file.'}</p>}<button className="dc-wide" onClick={close}>Back to the cinema →</button></>}
   ...
   {panel==='party'&&<><h2 id="panel-title" style={{margin:'0 0 4px', fontSize:'36px', lineHeight:'1.1'}}>Good stories.<br/>Better company.</h2><p style={{margin:'0 0 24px', fontSize:'13px'}}>One private cinema. Ten places. A shared moment.</p>
      <div className="dc-tabs">
        <button className={partyTab==='create'?'selected':''} onClick={()=>setPartyTab('create')}><UserPlus size={14}/> Create a party</button>
        <button className={partyTab==='join'?'selected':''} onClick={()=>setPartyTab('join')}><Link size={14}/> Join a party</button>
      </div>
      {partyTab==='create' && <>
        <label className="dc-field">YOUR NAME
          <div style={{display:'flex', gap:'16px', alignItems:'center'}}>
            <div style={{width:'42px',height:'42px',borderRadius:'50%',background:'#3f4135',border:'1px solid rgba(255,255,255,0.1)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:600,fontSize:'14px',color:'white'}}>{name.charAt(0).toUpperCase()}</div>
            <input style={{flex:1}} value={name} onChange={e=>setName(e.target.value)} />
          </div>
        </label>
        <label className="dc-field" style={{marginTop:'24px'}}>GIVE YOUR EVENING A NAME
          <input value={partyName} onChange={e=>setPartyName(e.target.value)} />
        </label>
        <div style={{borderBottom: '1px solid rgba(255,255,255,0.05)', margin:'24px 0'}}></div>
        <button className="dc-option-row" style={{display:'flex',alignItems:'center',gap:'16px',width:'100%',textAlign:'left',padding:'12px 0'}}>
          <Film size={20} style={{opacity:0.6}}/>
          <div style={{flex:1}}>
            <span className="dc-eyebrow" style={{display:'block',marginBottom:'4px',fontSize:'9px'}}>ON YOUR SCREEN</span>
            <strong style={{display:'block',fontWeight:500,fontSize:'13px',color:'white'}}>{media.filename?title:'Select a video'}</strong>
          </div>
          <span style={{opacity:0.6,fontSize:'12px',display:'flex',alignItems:'center',gap:'4px'}}>Change <ArrowRight size={14}/></span>
        </button>
        <div style={{borderBottom: '1px solid rgba(255,255,255,0.05)', margin:'24px 0'}}></div>
        <button className="dc-option-row" style={{display:'flex',alignItems:'center',gap:'16px',width:'100%',textAlign:'left',padding:'12px 0'}}>
          <Globe size={20} style={{opacity:0.6}}/>
          <div style={{flex:1}}>
            <strong style={{display:'block',fontWeight:500,fontSize:'13px',color:'white',marginBottom:'4px'}}>Pick from the TMDB collection</strong>
            <span style={{opacity:0.6,fontSize:'11px'}}>Search movies, series & anime, then choose your server</span>
          </div>
          <ArrowRight size={16} style={{opacity:0.6}}/>
        </button>
        <div style={{borderBottom: '1px solid rgba(255,255,255,0.05)', margin:'24px 0'}}></div>
        <div style={{display:'flex', gap:'16px', alignItems:'flex-start', marginBottom:'32px'}}>
          <Monitor size={22} style={{opacity:0.8, marginTop:'2px'}}/>
          <div>
            <strong style={{display:'block',fontSize:'14px',color:'white',marginBottom:'6px'}}>Online watch party.</strong>
            <p style={{margin:0,opacity:0.7,fontSize:'12px',lineHeight:'1.6'}}>Anyone with the room code can join from another device. Players, seats and the selected film stay in sync. Direct-video playback follows the host.</p>
          </div>
        </div>
        <button className="dc-wide-white" onClick={()=>setNotice('Watch party backend not connected yet')}><span style={{display:'flex',alignItems:'center',gap:'8px'}}><UserPlus size={16}/> Create watch party</span> <ArrowRight size={18}/></button>
      </>}
      {partyTab==='join' && <>
        <label className="dc-field">YOUR NAME
          <div style={{display:'flex', gap:'16px', alignItems:'center'}}>
        ...
        <label className="dc-field" style={{marginTop:'24px'}}>ROOM CODE OR INVITATION LINK
          <input placeholder="e.g. K7MN2A" value={partyCode} onChange={e=>setPartyCode(e.target.value)} />
        </label>
        <div style={{borderBottom: '1px solid rgba(255,255,255,0.05)', margin:'24px 0'}}></div>
        <div style={{display:'flex', gap:'16px', alignItems:'flex-start', marginBottom:'32px'}}>
          <Monitor size={22} style={{opacity:0.8, marginTop:'2px'}}/>
        ...
        <button className="dc-wide-white" onClick={()=>setNotice('Watch party backend not connected yet')}><span style={{display:'flex',alignItems:'center',gap:'8px'}}><Users size={16}/> Join the cinema</span> <ArrowRight size={18}/></button>
        <p style={{textAlign:'center', marginTop:'24px', opacity:0.6, fontSize:'11px'}}>No account. No upload. Keep the host tab open.</p>
      </>}
    </>}
