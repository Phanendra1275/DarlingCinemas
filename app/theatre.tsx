...
export type TheatreProps={seat:string|null;video?:HTMLVideoElement|null;youtubeId?:string|null;lite?:boolean;members?:Member[];service?:number;onSeat?:(seat:string)=>void;onNearbySeat?:(seat:string|null)=>void;onFps?:(fps:number)=>void;onWalk?:()=>void;moveRef?:React.MutableRefObject<{x:number;y:number;magnitude:number}>;reset?:number;zoom?:boolean;isTouch?:boolean;walk?:number;jacket?:string;cameraSettings?:CameraSettings;blocked?:boolean;quality?:string;reducedMotion?:boolean;highRefresh?:boolean;destination?:{id:string;request:number}|null;onRouteCancel?:()=>void;onServiceState?:(state:string)=>void;name?:string;gender?:string;};
export default function Theatre(props:TheatreProps){
 ...
 el.insertBefore(cssRenderer.domElement, renderer.domElement);
 const cssScene = new T.Scene();
 const iframeWrapper = document.createElement('div');
 iframeWrapper.style.width = '880px'; iframeWrapper.style.height = '495px'; iframeWrapper.style.backgroundColor = '#000'; iframeWrapper.style.pointerEvents = 'auto';
 const cssObject = new CSS3DObject(iframeWrapper);
 cssObject.scale.set(SCREEN.width / 880, SCREEN.height / 495, 1);
 ...
 cssObject.position.copy(room.screen.position); cssObject.position.z += 0.01; cssObject.rotation.copy(room.screen.rotation);
 ...
 const resize=()=>{const w=el.clientWidth,h=el.clientHeight;if(!w||!h)return;camera.aspect=w/h;camera.updateProjectionMatrix();renderer.setSize(w,h);cssRenderer.setSize(w,h);};const observer=new ResizeObserver(resize);observer.observe(el);resize();
 ...
 function frame(t:number){id=requestAnimationFrame(frame);if(!latest.current.highRefresh&&t-last<1000/60-.5)return;const dt=Math.min((t-last)/1000,.08);last=t;elapsed+=dt;const p=latest.current,cfg=p.cameraSettings??defaultCamera,blend=1-Math.exp(-dt*(p.reducedMotion?20:cfg.damping));
  ...
  if (p.youtubeId !== iframeWrapper.dataset.yt) {
    iframeWrapper.dataset.yt = p.youtubeId || '';
    if (p.youtubeId) {
      iframeWrapper.innerHTML = `<iframe width="100%" height="100%" src="https://www.youtube.com/embed/${p.youtubeId}?autoplay=1&controls=0&modestbranding=1" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen style="pointer-events: auto;"></iframe>`;
      (room.screen.material as T.MeshBasicMaterial).colorWrite = false;
      cssObject.visible = true;
    } else {
      iframeWrapper.innerHTML = '';
      (room.screen.material as T.MeshBasicMaterial).colorWrite = true;
  ...
  const dark=!!selected&&( (ready&&!video!.paused) || !!p.youtubeId );const lb=1-Math.exp(-dt*2);room.glow.color.lerp(new T.Color(dark?"#423823":"#e3c88a"),lb);room.hemi.intensity=T.MathUtils.lerp(room.hemi.intensity,dark?.075:1.05,lb);room.key.intensity=T.MathUtils.lerp(room.key.intensity,dark?.04:1.9,lb);room.lamps.forEach(l=>l.intensity=T.MathUtils.lerp(l.intensity,dark?.18:7,lb));
  if(dark){
  ...
  renderer.render(scene,camera);cssRenderer.render(cssScene,camera);count++;if(t-fpsAt>1000){p.onFps?.(Math.round(count*1000/(t-fpsAt)));count=0;fpsAt=t;}
 }
